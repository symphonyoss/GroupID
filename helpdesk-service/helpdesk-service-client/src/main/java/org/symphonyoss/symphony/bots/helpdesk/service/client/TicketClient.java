...
public class TicketClient {
  ...
  public Ticket getTicketByServiceStreamId(String serviceStreamId) {
    try {
      List<Ticket> ticketList = ticketApi.searchTicket(groupId, serviceStreamId, null).getTicketList();
  ...
  public Ticket getTicketByClientStreamId(String clientStreamId) {
    try {
      List<Ticket> ticketList = ticketApi.searchTicket(groupId, null, clientStreamId).getTicketList();
      if(!ticketList.isEmpty()) {
        return ticketList.get(0);
      }
    } catch (ApiException e) {
