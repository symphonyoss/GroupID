...
import org.symphonyoss.symphony.bots.helpdesk.bot.model.Error;
 ...
 *
 * Created by lpimentel on 01/12/2017.
 */
@ControllerAdvice
public class ApiExceptionHandler {
  ...
  public ResponseEntity<Error> handleUnsupportedMediaTypeException(Exception ex) {
    Error response =
        new Error().code(HttpStatus.UNSUPPORTED_MEDIA_TYPE.value()).message(ex.getMessage());
  ...
  public ResponseEntity<Error> handleMethodNotSupportedException(Exception ex) {
    Error response =
        new Error().code(HttpStatus.METHOD_NOT_ALLOWED.value()).message(ex.getMessage());
